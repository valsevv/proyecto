...
export default class HudScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.turnText = this.add
      .text(this.scale.width / 2, 20, "Conectando...", {
        fontSize: "20px",
        fill: "#ffffff",
    ...
    this.droneInfoText = this.add
      .text(this.scale.width / 2, 55, "", {
        fontSize: "14px",
        fill: "#cccccc",
    ...
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      if (!mainScene) return;
      if (this._onAttackAnimStart)
        mainScene.events.off("attackAnimStart", this._onAttackAnimStart, this);
      if (this._onAttackAnimImpact)
        mainScene.events.off("attackAnimImpact", this._onAttackAnimImpact, this);
  ...
  createActionButtons() {
    ...
    this.drawRadarButton(
      ...
      btnRadius,
      this.grayColor,
      this.radarColors.attackFrame,
      this.radarColors.attackGlow,
      1,
      0.2
    );
    ...
    this.attackBtn = this.add
      .rectangle(startX, y, btnWidth, btnHeight, 0x000000, 0.001)
    ...
    this.drawRadarButton(
      ...
      btnRadius,
      this.grayColor,
      this.radarColors.recallFrame,
      this.radarColors.recallGlow,
      1,
      0.2
    );

    this.recallBtnText = this.add
      .text(recallX, y, "REPLEGAR", {
        fontSize: "15px",
        fill: "#ffffff",
        fontFamily: this.btnFontFamily,
        stroke: "#000000",
    ...
    this.recallBtn = this.add
      .rectangle(recallX, y, btnWidth, btnHeight, 0x000000, 0.001)
      .setInteractive({ useHandCursor: true });
    ...
    this.endTurnBtnText = this.add
      .text(endTurnX, y, "FIN TURNO", {
        fontSize: "15px",
        fill: "#ffffff",
        fontFamily: this.btnFontFamily,
        stroke: "#000000",
    ...
    this.endTurnBtn = this.add
      .rectangle(endTurnX, y, btnWidth, btnHeight, 0x000000, 0.001)
      .setInteractive({ useHandCursor: true });
    ...
    this.saveExitBtnText = this.add
      .text(saveExitX, saveExitY, "💾 Guardar", {
        fontSize: "16px",
        fill: "#ffffff"
      })
      .setOrigin(0.5);

    this.saveExitBtn = this.add
      .rectangle(saveExitX, saveExitY, btnWidth, btnHeight, 0x000000, 0.001)
      .setInteractive({ useHandCursor: true });
  ...
  updateButtonStates() {
    ...
    const hasAmmoForAttack = !selectedDrone
      ? true
      : (selectedDrone.missiles ?? 0) > 0 &&
        (selectedDrone.droneType !== "Naval" || selectedDrone.canUseMissileAttack());
  ...
  onDeployPanelOpen(data) {
    ...
    const title = this.add
      .text(panelX + panelW / 2, panelY + padding, "🚁 Desplegar Drones", {
        fontSize: "14px",
        fill: "#7ecfff",
        fontStyle: "bold"
      })
    ...
    const closeText = this.add
      .text(panelX + panelW - 18, panelY + 17, "✕", {
        fontSize: "12px",
        fill: "#ffaaaa"
      })
      .setOrigin(0.5)
    ...
    const closeHit = this.add
      .rectangle(panelX + panelW - 18, panelY + 17, 22, 22, 0x000000, 0.001)
      .setInteractive({ useHandCursor: true })
      .setDepth(203);
    ...
    for (let i = 0; i < drones.length; i++) {
      ...
      const btnLabel = this.add
        .text(btnX + btnW / 2, rowY + rowH / 2, "Desplegar", {
          fontSize: "12px",
          fill: "#ffffff"
        })
        .setOrigin(0.5)
      ...
      const hit = this.add
        .rectangle(btnX + btnW / 2, rowY + rowH / 2, btnW, btnH, 0x000000, 0.001)
        .setInteractive({ useHandCursor: true })
        .setDepth(203);
  ...
  update() {
    ...
    if (this.isMyTurn && mainScene.selectedDrone) {
      ...
      const hasAmmoForAttack =
        (drone.missiles ?? 0) > 0 && (drone.droneType !== "Naval" || drone.canUseMissileAttack());
      ...
      const undeployedCount = (mainScene.myDrones || []).filter(
        (d) => !d.deployed && d.isAlive()
      ).length;
      if (undeployedCount > 0) {
        ...
        this.droneInfoText.setText(
          `Portadrones seleccionado  |  Movimiento máx: ${maxMove} casillas`
        );
      }
    ...
    const tracked = visibleUnits.map((unit) => ({
      x: unit.x,
      y: unit.y,
      color: unit.isSelected ? 0xffff00 : TEAM_COLORS[unit.playerIndex]
    }));

    this.minimap.update(mainScene.cameras.main, tracked);

    // Only show deployed drones in the fuel list
    const myAliveDrones = (mainScene.myDrones || []).filter(
      (drone) => drone.isAlive() && drone.deployed
    );
    ...
    if (this.deployPanelElements.length === 0) {
      ...
      if (myAliveDrones.length) {
        ...
        const header =
          undeployedCount > 0 ? `Combustible  (${undeployedCount} en hangar)` : "Combustible";
