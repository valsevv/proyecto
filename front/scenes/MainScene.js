...
const DEFAULT_MAX_MOVE_DISTANCE = 6; // hexes per turn
const DEFAULT_MAX_CARRIER_MOVE_DISTANCE = 3; // hexes per turn (parametrizable)
const DEFAULT_MAX_MANUAL_ATTACK_DISTANCE = 15; // max range in hexes for manual missile aiming (parametrizable)

// Vision ranges (in hexes). Aereo must see 2x Naval.
// Adjust NAVAL_VISION_RANGE to tune both.
const DEFAULT_NAVAL_VISION_RANGE = 2;
const DEFAULT_AEREO_VISION_RANGE = DEFAULT_NAVAL_VISION_RANGE * 2;
...
export default class MainScene extends Phaser.Scene {
  constructor() {
    ...
    this.navalVisionRange = DEFAULT_NAVAL_VISION_RANGE;
    this.aereoVisionRange = DEFAULT_AEREO_VISION_RANGE;
    ...
    this.maxCarrierMoveDistance = DEFAULT_MAX_CARRIER_MOVE_DISTANCE;
    this.maxManualAttackDistance = DEFAULT_MAX_MANUAL_ATTACK_DISTANCE;
  }
  ...
  create(data) {
    ...
    this.input.on("pointermove", (pointer) => {
      ...
      if (
        ...
        this.selectedDrone &&
        this.selectedDrone.droneType === "Naval" &&
        !pointer.isDown &&
        this.isMyTurn
      ) {
    ...
    this.input.on("pointerup", (pointer) => {
      ...
      if (this.selectedCarrier) {
        ...
        if (distance > this.selectedCarrier.maxMoveDistance) {
          console.warn(
            `Carrier move too far: ${distance} hexes (max ${this.selectedCarrier.maxMoveDistance})`
          );
          return;
        }
        ...
        if (
          this.isPositionOccupied(nearest.x, nearest.y, 15, this.selectedCarrier, {
            checkDrones: false
          })
        ) {
      ...
      if (distance > this.maxMoveDistance) {
        console.warn(`Move too far: ${distance} hexes (max ${this.maxMoveDistance})`);
        return;
      }
  ...
  createCarrierForPlayer(playerIndex, side, spawnHint = null) {
    const basePosition =
      spawnHint && typeof spawnHint.x === "number" && typeof spawnHint.y === "number"
    ...
    const carrier = {
      ...
      playerIndex,
      isLocal,
      maxMoveDistance: this.maxCarrierMoveDistance,
      type: "carrier",
      side,
      direction: 0,
      health: 5,
  ...
  createDronesFromState(state) {
    ...
    if (typeof state?.droneMaxMoveDistance === "number") {
      this.maxMoveDistance = Math.max(1, state.droneMaxMoveDistance);
    }
    if (typeof state?.carrierMaxMoveDistance === "number") {
      this.maxCarrierMoveDistance = Math.max(1, state.carrierMaxMoveDistance);
    }
    if (typeof state?.missileMaxDistance === "number") {
      this.maxManualAttackDistance = Math.max(1, state.missileMaxDistance);
    }
    ...
    for (const player of state.players) {
      ...
      const spawnHint =
        typeof player.carrierX === "number" && typeof player.carrierY === "number"
      ...
      if (this.carriers[player.playerIndex]) {
        ...
        this.carriers[player.playerIndex].health =
          player.carrierHealth ?? this.carriers[player.playerIndex].maxHealth;
        this.carriers[player.playerIndex].destroyed =
          player.carrierDestroyed === true || this.carriers[player.playerIndex].health <= 0;
  ...
  executeAttack(targetUnit) {
    ...
    if (this.selectedDrone.droneType === "Aereo") {
      ...
      console.log(
        "[MainScene] Aereo attack distance check:",
        distance,
        "range:",
        this.selectedDrone.attackRange
      );
      if (distance > (this.selectedDrone.attackRange ?? 0)) {
  ...
  setManualAttackLine(hexCenter) {
    ...
    const maxDistancePixels = this.hexGrid.size * Math.sqrt(3) * this.maxManualAttackDistance;
  ...
  drawMissileGuides() {
    ...
    if (this.manualTargetHex) {
      ...
      this.hexGrid.drawFilledHex(
        this.missileGuideGraphics,
        this.manualTargetHex.x,
        this.manualTargetHex.y,
        0x29b6f6,
        0.28
  ...
  updateHexHighlight(pointer) {
    ...
    if (
      (!this.selectedDrone && !this.selectedCarrier) ||
      !this.isMyTurn ||
      this.actionMode !== MODE_MOVE
    ) {
      this.lastHighlightedHex = null;
    ...
    const nearest = this.hexGrid.getNearestCenter(pointer.worldX, pointer.worldY);
    const selectedUnitSprite = this.selectedCarrier
      ? this.selectedCarrier.sprite
      : this.selectedDrone.sprite;
    ...
    const distance = this.hexGrid.getHexDistance(
      selectedUnitSprite.x,
      selectedUnitSprite.y,
      nearest.x,
      nearest.y
    );
